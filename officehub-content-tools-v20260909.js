(function () {
  ...
  function session() {
    try {
      ...
      return value && (value.access_token ? value : value.currentSession);
    } catch (_) { return null; }
  }
  async function key() {
    ...
    const match = text.match(/eyJ[a-zA-Z0-9_-]{80,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}/);
  ...
  async function recipientBox() {
    const box=document.createElement('section'); box.className='oh-tools'; box.dataset.oh='notice-recipients'; box.innerHTML='<h3>Specific recipients</h3><p class="oh-help">Choose departments, then keep everyone selected or remove individual people.</p><div class="oh-depts">Loading departments…</div><div class="oh-actions"><button type="button" class="oh-btn" data-all>Select all users</button><button type="button" class="oh-btn" data-clear>Clear selection</button><span class="oh-count">0 people selected</span></div><div class="oh-people"></div><div class="oh-error"></div>';
    try {
      const [deps, memberships, profiles] = await Promise.all([api('/rest/v1/departments?select=id,name&is_active=eq.true&order=name'),api('/rest/v1/user_departments?select=user_id,department_id'),api('/rest/v1/profiles?select=id,full_name,display_name,email&status=eq.active&order=full_name')]);
  ...
  async function enhance() {
    ...
    for(const form of document.querySelectorAll('form')){
      ...
      if((path.includes('notice')||path.includes('announcement'))&&!form.querySelector('[data-oh="notice-files"]')){const r=await recipientBox();target(form).before(r);target(form).before(filesBox('notice'));}
  ...
  const nativeFetch=window.fetch.bind(window); window.fetch=async function(input,init){let url=typeof input==='string'?input:input.url;let method=(init?.method||(typeof input!=='string'&&input.method)||'GET').toUpperCase();let next=init;if(method==='POST'&&/\/rest\/v1\/(announcements|projects)(\?|$)/.test(url)){next=Object.assign({},init,{headers:Object.assign({},init?.headers||{},{Prefer:'return=representation'})});}const res=await nativeFetch(input,next);if(res.ok&&method==='POST'){const m=url.match(/\/rest\/v1\/(announcements|projects)(\?|$)/);if(m){try{const rows=await res.clone().json();const row=Array.isArray(rows)?rows[0]:rows;if(row?.id){if(m[1]==='announcements'){const s=session();if(state.recipients.size)await api('/rest/v1/announcement_recipients',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify([...state.recipients].map(user_id=>({announcement_id:row.id,user_id,created_by:s.user.id})))});await uploadFiles('notice',row.id,state.noticeFiles);}else await uploadFiles('project',row.id,state.projectFiles);}}catch(e){console.error('OfficeHub attachments:',e);}}}return res;};
