(function () {
  ...
  function session() {
    try {
      const raw = localStorage.getItem('auth-token-wipahs-officehub') || localStorage.getItem(`sb-${PROJECT}-auth-token`);
      const value = raw && JSON.parse(raw);
      return value && (value.access_token ? value : value.currentSession || value.session);
    } catch (_) { return null; }
  }
  async function key() {
