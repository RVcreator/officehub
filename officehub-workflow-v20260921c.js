(function(){
'use strict';
const RELEASE='20260921c',PROJECT='wxjjrfhahxqhbixaviwm',API=`https://${PROJECT}.supabase.co`,previousFetch=window.fetch.bind(window),originalScrollIntoView=Element.prototype.scrollIntoView;
...
let workflowTimer=0;
function scheduleInit(){clearTimeout(workflowTimer);workflowTimer=setTimeout(init,120)}
addEventListener('popstate',scheduleInit);addEventListener('oh-routechange',scheduleInit);
new MutationObserver(scheduleInit).observe(document.getElementById('root')||document.documentElement,{childList:true,subtree:true});
scheduleInit();
