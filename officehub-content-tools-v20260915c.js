(function () {
  ...
  function noticeForm() {
    return [...document.querySelectorAll('form')].find(form =>
      form.querySelector('input[placeholder*="Notice title" i]') &&
      form.querySelector('textarea[placeholder*="Notice content" i]')
    );
  }
  function dedupe(selector, form) {
    const nodes = [...document.querySelectorAll(selector)];
    const keep = nodes.find(node => node.closest('form') === form) || null;
  ...
  async function enhance() {
    ...
    try {
      ...
      if(path.includes('notice')||path.includes('announcement')){
        const form=noticeForm();
        if(form){
          let recipients=dedupe('[data-oh="notice-recipients"]',form);
          let files=dedupe('[data-oh="notice-files"]',form);
          if(!recipients||!files){
            form.dataset.ohEnhancing='1';
            if(!recipients){recipients=await recipientBox();target(form).before(recipients);}
            if(!files){files=filesBox('notice');target(form).before(files);}
            delete form.dataset.ohEnhancing;
          }
        } else {
          document.querySelectorAll('[data-oh="notice-recipients"],[data-oh="notice-files"]').forEach(node=>node.remove());
        }
      }
      if(path.includes('project')){
        for(const form of document.querySelectorAll('form')){
          const text=form.innerText.toLowerCase();
          if(!text.includes('search')&&!form.querySelector('[data-oh="project-files"]'))target(form).before(filesBox('project'));
        }
      }
    } finally {
      enhancing = false;
