    ...
    list.onclick = async event => {
      ...
      try {
        if (event.target.closest('[data-save]')) {
          ...
          await api(`/rest/v1/task_subtasks?id=eq.${encodeURIComponent(row.dataset.row)}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ title, updated_at: new Date().toISOString() }) });
        } else if (event.target.closest('[data-delete]')) {
          await api(`/rest/v1/task_subtasks?id=eq.${encodeURIComponent(row.dataset.row)}`, { method: 'DELETE', headers: { Prefer: 'return=minimal' } });
        } else return;
        await loadEdit(taskId, true);
      } catch (err) { error.textContent = err.message; }
    };
